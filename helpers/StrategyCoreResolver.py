...
class StrategyCoreResolver:
    ...
    # set the SnapshotManager instance is the manager of this StrategyCoreResolver
    ...
    def add_strategy_snap(self, calls, entities=None):
        ...
        calls.append(
            Call(
                strategy.address,
                [func.strategy.getTimelySupplyPosition],
                [["strategy.getTimelySupplyPosition", as_wei]],
            )
        )
        calls.append(
            Call(
                strategy.address,
                [func.strategy.getTimelyBorrowPosition],
                [["strategy.getTimelyBorrowPosition", as_wei]],
            )
        )
        calls.append(
            Call(
                strategy.address,
                [func.strategy.collateralTarget],
                [["strategy.collateralTarget", as_wei]],
            )
        )
        calls.append(
            Call(
                strategy.address,
                [func.strategy.minWant],
                [["strategy.minWant", as_wei]],
            )
        )
    ...
    def confirm_withdraw(self, before, after, params, tx):
        ...
        if expected_want > before.balances("want", "sett"):
            ...
            # Make Change for the withdraw condition
